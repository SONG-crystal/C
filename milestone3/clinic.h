...
struct Phone
{
    char description[PHONE_DESC_LEN + 1];   //char *description
    char number[PHONE_LEN + 1]; 
};


struct Patient //pets[7]
{
    int patientNumber;
    char name[NAME_LEN + 1];  //char *name
    struct Phone phone;
};
...
// Add an appointment record to the appointment array //////////이거 하는 중, 틀릴수도있음
// Todo:
...
//Sort the total time using Bubble sort
void sortData(struct Appointment appointment[], int max);

//Calculate the total minute from year, month, day, hour
void totalMinute(struct Appointment totalMin[], int max);

//Calculate the only minute from totalMinute 
